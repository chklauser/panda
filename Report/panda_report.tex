...
We introduced a block system. Each block can be one of four types. To achieve maintainable code we designed three layers. These block types are described in the Specification section and the layers in the Design section.

\subsection{Requirements}

Each requirement which is listed is also implemented. The software elements which are involved can be found in the test class\newline Panda.Test/Integration/Specification.cs. The test method is noted to each requirement.
...
The VFS Core is divided in three layers. Each of it accesses the layer below and ensures good maintenanceability and readability.

\subsubsection{I/O layer}

On this level we store raw data. The methods related to this abstraction level are stored in Panda.Core/Core/IO.

\subsubsection{Block layer}

We have abstract blocks of uniform size. They are specified in the section Specification. The class BlockManager manages the access to these blocks. It communicates with the I/O layer.\\
